"""
AUTHOR : Nicolas Unger & Khaled Al Moulla
...
def validate_inputs(df, error_significant_figures, error_suffix, error):
    """
    Validate the input parameters for the pd2ltx function
    """
    if not isinstance(df, pd.DataFrame):
        raise ValueError("df has to be of type pandas.DataFrame")
    if not isinstance(error_significant_figures, int):
        raise ValueError("error_significant_figures has to be an int")

    if error == "symmetrical":
        if not isinstance(error_suffix, str):
            raise ValueError("error_suffix has to be of type str with symmetrical errors")
    elif error == "asymmetrical":
        if not isinstance(error_suffix, list):
            raise ValueError("error_suffix has to be of type list with asymmetrical errors")
        if len(error_suffix) != 2:
            raise ValueError("error_suffix has to be of length 2 with upper error and lower error respectively")


def strrep_func(strval, err, precision):
    """
    Helper function to create string representation of a value with error
    """
    sym = isinstance(err, float)
    if sym:
        if precision < 0:
            strrep = f"${strval}\pm{round(err, precision):g}$"
        elif precision > 4:
            errpm_str = np.format_float_scientific(
                round(err, precision), unique=True, exp_digits=1, min_digits=1
            )
            strrep = f"${strval}\pm{errpm_str}$"
        else:
            strrep = f"${strval}\pm{err:.{precision}f}$"
    else:
        errup, errlo = err
        if precision < 0:
...
def construct_latex_table(df, error, error_suffix, error_significant_figures, var_idx, labels):
    """
    Construct the LaTeX table DataFrame
    """

    # Get size of DataFrame
    Nrow, Ncol = df.shape
    # Extract the labels of the columns
    labels = df.columns.values.tolist()
    # Which columns have numerical values
    var_idx = [idx for idx, lbl in enumerate(labels) if "error" not in lbl]
    ...
    for i in range(Nrow):
        for j in var_idx:
            ...
            if precision < 0:
                strval = f"{round(df.loc[i, labels[j]], dec):g}"
            elif precision > 4:
                strval = np.format_float_scientific(
                    round(df.loc[i, labels[j]], dec), unique=True, exp_digits=1, min_digits=1
                )
            else:
                strval = f"{df.loc[i, labels[j]]:.{precision}f}"

            ltdf.loc[i, labels[j]] = strrep_func(strval, err_cols, precision)
...
def pd2ltx(
    ...
    **kwargs : dict
    ...
    # Validate the inputs
    validate_inputs(df, error_significant_figures, error_suffix, error)
    ...
    ltdf = construct_latex_table(df, error, error_suffix, error_significant_figures, var_idx, labels)

    # Render the DataFrame as a LaTeX table and return the resulting string
    return ltdf.to_latex(**kwargs)
